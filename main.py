...
def run_gridsearch(n_expt : int, delta_t : float, noise_level : float, parameters : dict, perform_grid_search : bool = True, 
    ...
    for status in ["without constraints", "with constraints", "with stoichiometry"]:
        ...
        if perform_grid_search:
            ...
            opt = HyperOpt(features, target, features_clean, target_clean, time_span, parameters, Optimizer_casadi(solver_dict = solver_dict), 
            ...
            opt.plot(filename = f"saved_data\Gridsearch_{name}_{status}_noise{noise_level}_eniter{ensemble_iterations}.html", title = f"{status} and {noise_level} noise")
...
if __name__ == "__main__": 
    ...
    # plot_adict(noise_level, adict_noise, x_label = "noise_level")
    ########################################################################################################################
    # effect of amount of data on model performance
    ...
    for expt in experiments:
        run_gridsearch(expt, 0.01, noise_level= 0, parameters = ensemble_params, ensemble_iterations=1000, max_workers = 2, seed = 10, 
    ...
    for sample in sampling:
        run_gridsearch(15, sample, noise_level= 0, parameters = ensemble_params, ensemble_iterations=1000, max_workers = 2, seed = 10, 
